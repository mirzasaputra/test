...
$pdf->Cell(0, 5, 'LAPORAN STOK IN/OUT', 0, 1, 'C');
...
} else if ($jenis == "Stok Masuk") {

    $sql = "SELECT d.id_stok, a.barcode, a.nama_barang, d.jenis, d.nilai, d.keterangan, SUBSTRING(d.tanggal, 1, 10) AS tanggal, d.jml, c.satuan FROM  barang a, satuan c, stok d        
    WHERE a.id_satuan = c.id_satuan AND d.id_barang = a.id_barang AND SUBSTRING(d.tanggal, 1, 10) BETWEEN '$awal' AND '$akhir' AND d.jenis = 'Stok Masuk'";
} else {

    $sql = "SELECT d.id_stok, a.barcode, a.nama_barang, d.jenis, d.nilai, d.keterangan, SUBSTRING(d.tanggal, 1, 10) AS tanggal, d.jml, c.satuan FROM  barang a, satuan c, stok d        
    WHERE a.id_satuan = c.id_satuan AND d.id_barang = a.id_barang AND SUBSTRING(d.tanggal, 1, 10) BETWEEN '$awal' AND '$akhir' AND d.jenis = 'Stok Keluar'";
}
